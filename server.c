...
void whisper_message(const char *message, const char *username, int sender_sock) {
    const char* sender_username = find_username_by_socket(sender_sock);
    char formatted_message[MAX_INPUT_LENGTH + MAX_USERNAME_LENGTH + 3]; // Enough space for the message and the username

    pthread_mutex_lock(&clients_mutex);
    if (sender_username) {
        // Format the message to indicate it's a private whisper
        snprintf(formatted_message, sizeof(formatted_message), "%s (whisper): %s", sender_username, message);
